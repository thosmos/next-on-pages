...
// src/index.ts
var prepVercel = async () => {
  try {
    await (0, import_promises.stat)(".vercel/project.json");
  } catch {
    await (0, import_promises.mkdir)(".vercel", { recursive: true });
...
var transform = async ({
  ...
  let config;
  try {
    config = JSON.parse(await (0, import_promises.readFile)(".vercel/output/config.json", "utf8"));
  } catch {
    console.error(
  ...
  const tmpFunctionsDir = (0, import_path.join)((0, import_os.tmpdir)(), Math.random().toString(36).slice(2));
  const tmpWebpackDir = (0, import_path.join)((0, import_os.tmpdir)(), Math.random().toString(36).slice(2));
  const invalidFunctions = [];
  const webpackChunks = /* @__PURE__ */ new Map();
  const walk = async (dir) => {
    const files = await (0, import_promises.readdir)(dir);
    await Promise.all(
      files.map(async (file) => {
        const filepath = (0, import_path.join)(dir, file);
        const isDirectory = (await (0, import_promises.stat)(filepath)).isDirectory();
        const relativePath = (0, import_path.relative)(functionsDir, filepath);
        if (isDirectory && filepath.endsWith(".func")) {
          const name = relativePath.replace(/\.func$/, "");
          const functionConfigFile = (0, import_path.join)(filepath, ".vc-config.json");
          let functionConfig;
          try {
            let contents2 = await (0, import_promises.readFile)(functionConfigFile, "utf8");
            functionConfig = JSON.parse(contents2);
          } catch {
          ...
          const functionFile = (0, import_path.join)(filepath, functionConfig.entrypoint);
          let functionFileExists = false;
          try {
            await (0, import_promises.stat)(functionFile);
            ...
            for (const objectOfChunks of expressions) {
              for (const chunkExpression of objectOfChunks) {
                ...
                if (key in webpackChunks) {
                  if (webpackChunks.get(key) !== generate(chunkExpression.value)) {
                    console.error(
                ...
                const chunkFilePath = (0, import_path.join)(tmpWebpackDir, `${key}.js`);
                const newValue = {
          ...
          const newFilePath = (0, import_path.join)(tmpFunctionsDir, `${relativePath}.js`);
          await (0, import_promises.mkdir)((0, import_path.dirname)(newFilePath), { recursive: true });
          await (0, import_promises.writeFile)(newFilePath, contents);
          functionsMap.set(
            (0, import_path.relative)(functionsDir, filepath).slice(0, -".func".length),
            newFilePath
          );
        } else if (isDirectory) {
          await walk(filepath);
  ...
  for (const [chunkIdentifier, code] of webpackChunks) {
    const chunkFilePath = (0, import_path.join)(tmpWebpackDir, `${chunkIdentifier}.js`);
    await (0, import_promises.mkdir)((0, import_path.dirname)(chunkFilePath), { recursive: true });
    await (0, import_promises.writeFile)(chunkFilePath, `export default ${code}`);
  }
  ...
  try {
    middlewareManifest = JSON.parse(
      await (0, import_promises.readFile)(".next/server/middleware-manifest.json", "utf8")
    );
  } catch {
  ...
  const hydratedMiddleware = /* @__PURE__ */ new Map();
  const hydratedFunctions = /* @__PURE__ */ new Map();
  ...
  const functionsFile = (0, import_path.join)(
    (0, import_os.tmpdir)(),
  ...
  await (0, import_promises.writeFile)(
    functionsFile,
    `
    export const __FUNCTIONS__ = {${[...hydratedFunctions.entries()].map(
      ([name, { matchers, filepath }]) => `"${name}": { matchers: ${JSON.stringify(
        matchers
      )}, entrypoint: require('${filepath}')}`
    ).join(",")}};
      
      export const __MIDDLEWARE__ = {${[...hydratedMiddleware.entries()].map(
      ([name, { matchers, filepath }]) => `"${name}": { matchers: ${JSON.stringify(
        matchers
      )}, entrypoint: require('${filepath}')}`
    ).join(",")}};`
  ...
  await (0, import_esbuild.build)({
    ...
    bundle: true,
    inject: [
      (0, import_path.join)(__dirname, "../templates/_worker.js/globals.js"),
      functionsFile
    ],
    target: "es2021",
    platform: "neutral",
